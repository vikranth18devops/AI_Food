...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  private logger = new StructuredLogger('recommendation-service:rabbitmq');
  ...
  public readonly queueName = 'recommendation_queue';
  ...
  private async connectWithRetry(retries = 5, delayMs = 3000) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        await this.channel.bindQueue(dlqName, this.dlxExchangeName, 'HEALTH_ANALYSIS_COMPLETED');

        await this.channel.assertQueue(this.queueName, {
          durable: true,
          arguments: {
            'x-dead-letter-exchange': this.dlxExchangeName,
            'x-dead-letter-routing-key': 'HEALTH_ANALYSIS_COMPLETED',
          },
        });
        await this.channel.bindQueue(this.queueName, this.exchangeName, 'HEALTH_ANALYSIS_COMPLETED');
    ...
    await this.channel.consume(
      ...
      async (msg: any) => {
        ...
        try {
          ...
          this.logger.error('Error processing message in recommendation consumer', err);
