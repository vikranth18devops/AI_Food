...
export class RabbitMQService implements OnModuleInit, OnModuleDestroy {
  private logger = new StructuredLogger('nutrition-service:rabbitmq');
  ...
  public readonly queueName = 'nutrition_queue';
  ...
  private async connectWithRetry(retries = 5, delayMs = 3000) {
    for (let i = 0; i < retries; i++) {
      try {
        ...
        await this.channel.bindQueue(dlqName, this.dlxExchangeName, 'FOOD_RECOGNIZED');

        await this.channel.assertQueue(this.queueName, {
          durable: true,
          arguments: {
            'x-dead-letter-exchange': this.dlxExchangeName,
            'x-dead-letter-routing-key': 'FOOD_RECOGNIZED',
          },
        });
        await this.channel.bindQueue(this.queueName, this.exchangeName, 'FOOD_RECOGNIZED');
    ...
    await this.channel.consume(
      ...
      async (msg: any) => {
        ...
        try {
          ...
          this.logger.error('Error processing message in nutrition service consumer', err);
